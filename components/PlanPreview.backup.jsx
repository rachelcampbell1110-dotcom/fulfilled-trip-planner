...
export default function PlanPreview({ plan, ai, loadingAi = false, onManualSave, onClearSavedDraft, saveStatusText, saveStatusColor }) {
  ...
  const manualSaveHandler = typeof onManualSave === 'function' ? () => onManualSave(localPlan) : () => {};

  const clearSavedDraftHandler = typeof onClearSavedDraft === 'function' ? onClearSavedDraft : () => {};

  const draftStatusText = saveStatusText || 'Progress auto-saves on this device.';

  const draftStatusColor = saveStatusColor || '#4b5563';

  const saveSuccess = draftStatusText === 'Saved!';

  const saveButtonStyle = saveSuccess

    ? { ...btnStyle, background: '#d1fae5', color: '#047857', borderColor: '#34d399' }

    : { ...btnStyle, background: '#f8fafc', color: THEME_PRIMARY_DARK, borderColor: THEME_PRIMARY_DARK };

  const clearButtonStyle = { ...btnStyle, background: '#fef2f2', color: '#b91c1c', borderColor: '#fca5a5' };



  const childAgesRaw = basics?.travelers?.ages ?? basics?.travelers?.agesChildren ?? [];

  const childAges = Array.isArray(childAgesRaw) ? childAgesRaw.filter((age) => typeof age === "number") : [];

  const hasKidTravelers = (basics?.travelers?.children ?? 0) > 0;

  const hasInfantTravelers = Boolean(basics?.hasInfantOrToddler);

  const youngestChild = childAges.length ? Math.min(...childAges) : null;

  let travelerSummaryNote = "";

  if (!hasKidTravelers) {

    travelerSummaryNote = "Adults-only getaway. Your packing list stays lean and skips kid gear.";

  } else if (hasInfantTravelers) {

    travelerSummaryNote = "Includes infant/toddler prep like crib confirmations, diaper restocks, and nap-friendly packing.";

  } else if (youngestChild !== null) {

    if (youngestChild <= 6) {

      travelerSummaryNote = "Family trip with big kids - expect kid-ready snacks, comfort items, and shared adventure ideas.";

    } else {

      travelerSummaryNote = "Family trip with older kids - lists focus on versatile layers and group-friendly plans.";
...
<html><head><meta charset=\"utf-8\"><title>${name} packing list</title><style>body{font-family:system-ui,Arial;padding:16px;}h1{font-size:20px;margin-bottom:12px;}ul{padding-left:18px;}li{margin:6px 0;}</style></head><body><h1>${name} packing list</h1><ul>${items.map((item) => `<li>${item}</li>`).join("")}</ul></body></html>`;
  ...
  return (
    ...
    <section id="trip-plan" ref={sectionRef} style={{ ...card, marginTop: 0, borderRadius: 24, padding: 24 }}>
      ...
      <div style={{ ...card, marginTop: 16, background: "linear-gradient(90deg, rgba(86, 144, 150, 0.08) 0%, #ffffff 100%)", borderColor: "#b7d6da" }}>

        <div style={{ fontWeight: 700, marginBottom: 8 }}>How It Works</div>

        <p style={{ margin: 0 }}>

          Now that your plan is created, you'll see a customized packing list, to-do list, and helpful reminders.

          Add items to the top of each list, remove tasks with the  button, and check them off as you go.

          To keep everything on track, save your progress at the bottom of the page and return anytime to pick up where you left off.
      ...
      <div style={{ display: "grid", gap: 20, gridTemplateColumns: "repeat(auto-fit, minmax(280px, 1fr))" }}>
        ...
        <div style={{ ...card, marginTop: 0 }}>
          ...
          {travelerSummaryNote && (
            ...
            <p style={{ marginTop: 10, fontSize: 13, color: '#4b5563' }}>{travelerSummaryNote}</p>

          )}

          {localPlan.ai_blurb && <p style={{ marginTop: travelerSummaryNote ? 6 : 10, fontStyle: "italic" }}>{localPlan.ai_blurb}</p>}
              ...
              <li key={`smh-${idx}`} style={itemRowStyle}>
                ...
                <label style={{ display: "inline-flex", gap: 6, alignItems: "center", flex: "1 1 auto" }}>
                  ...
                  <input
                    ...
                    checked={Boolean(checkedItems[makeChecklistKey("smartMustHaves", item)])}


                    onChange={() => toggleChecklistItem(makeChecklistKey("smartMustHaves", item))}
                        ...
                        <li key={idx} style={itemRowStyle}>
                          ...
                          <label style={{ display: "inline-flex", gap: 6, alignItems: "center", flex: "1 1 auto" }}>
                            ...
                            <input
                              ...
                              checked={Boolean(checkedItems[makeChecklistKey("packing", "person", name, item)])}


                              onChange={() => toggleChecklistItem(makeChecklistKey("packing", "person", name, item))}
                  ...
                  <li key={idx} style={itemRowStyle}>
                    ...
                    <label style={{ display: "inline-flex", gap: 6, alignItems: "center", flex: "1 1 auto" }}>
                      ...
                      <input
                        ...
                        checked={Boolean(checkedItems[makeChecklistKey("packing", "combined", item)])}


                        onChange={() => toggleChecklistItem(makeChecklistKey("packing", "combined", item))}
              ...
              <li key={`skip-${i}`} style={itemRowStyle}>
                ...
                <label style={{ display: "inline-flex", gap: 6, alignItems: "center", flex: "1 1 auto" }}>
                  ...
                  <input
                    ...
                    checked={Boolean(checkedItems[makeChecklistKey("overpack", "skip", t)])}


                    onChange={() => toggleChecklistItem(makeChecklistKey("overpack", "skip", t))}
              ...
              <li key={`lm-${i}`} style={itemRowStyle}>
                ...
                <label style={{ display: "inline-flex", gap: 6, alignItems: "center", flex: "1 1 auto" }}>
                  ...
                  <input
                    ...
                    checked={Boolean(checkedItems[makeChecklistKey("overpack", "lastMinute", t)])}


                    onChange={() => toggleChecklistItem(makeChecklistKey("overpack", "lastMinute", t))}
              ...
              <li key={`hp-${i}`} style={itemRowStyle}>
                ...
                <label style={{ display: "inline-flex", gap: 6, alignItems: "center", flex: "1 1 auto" }}>
                  ...
                  <input
                    ...
                    checked={Boolean(checkedItems[makeChecklistKey("overpack", "housePrep", t)])}


                    onChange={() => toggleChecklistItem(makeChecklistKey("overpack", "housePrep", t))}
                ...
                <li key={i}>
                  ...
                  <label style={{ display: "inline-flex", gap: 6, alignItems: "center" }}>
                    ...
                    <input
                      ...
                      checked={Boolean(checkedItems[makeChecklistKey("lodging", "infantToddler", t)])}


                      onChange={() => toggleChecklistItem(makeChecklistKey("lodging", "infantToddler", t))}
                        ...
                        <li key={taskIdx} style={itemRowStyle}>
                          ...
                          <label style={{ display: "inline-flex", gap: 6, alignItems: "center", flex: "1 1 auto" }}>
                            ...
                            <input
                              ...
                              checked={Boolean(checkedItems[makeChecklistKey("timeline", dayLabel, task)])}


                              onChange={() => toggleChecklistItem(makeChecklistKey("timeline", dayLabel, task))}
      ...
      <div className="__no-print" style={{ marginTop: 24, display: "flex", gap: 12, flexWrap: "wrap", justifyContent: "flex-end" }}>

        <button type="button" className="__actionbtn" onClick={manualSaveHandler} style={saveButtonStyle}>

          {saveSuccess ? "Saved!" : "Save progress"}

        </button>

        <button type="button" className="__actionbtn" onClick={clearSavedDraftHandler} style={clearButtonStyle}>
      ...
      <div className="__no-print" style={{ marginTop: 6, fontSize: 12, color: draftStatusColor, textAlign: 'right' }}>
